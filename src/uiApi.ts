...
export async function listBranches(repositoryPath:string, repositoryId?:string, ctx:UiContext={}){ if(repositoryId){ const listed=await listRepositories(ctx); const repo=listed.repositories.find(r=>r.id===repositoryId); if(!repo) throw new CloneRepositoryError("repository_not_found","GitHub repository not found."); return githubBranches(repo,ctx.env??process.env,ctx.githubFetch,ctx.githubExec); } return {branches: await listRepositoryBranches(repositoryPath)}; }
...
export async function startRun(_input:{projectId?:string; task?:string; baseBranch?:string; prTarget?:string; prTargetBranch?:string; repositoryPath?:string}, _ctx:UiContext={}) { throw new Error("UI start is disabled for AutoCodex v2; use CLI: npm run catos -- run --project <id> --task-package <directory>"); }
...
function isSafeRel(p:string){ return !!p && !path.isAbsolute(p) && !p.split(/[\\/]+/).includes(".."); }
function textOutput(file:string){ return [".md",".txt",".json",".diff",".log",".ts",".tsx",".js",".css",".html",".yaml",".yml"].includes(path.extname(file)); }
async function resolveRunScopedOutput(runId:string, requested:string, ctx:UiContext={}){ const runDir=path.resolve(runsOf(ctx),runId); if(!isSafeRel(requested)) throw new Error("Path traversal rejected"); const system=await buildUiSystemState(runDir); const stepMeta=(system.steps||[]).flatMap(s=>[s.report,s.resultFile]).find(o=>o?.path===requested); const meta=(system.outputs||[]).find(o=>o.path===requested) ?? stepMeta; const final=JSON.parse(await readFile(path.join(runDir,"final-result.json"),"utf8").catch(()=>"{}")); const workspace=String(final.workspacePath ?? final.finalWorkspacePath ?? ""); const legacyRunOutput = ["FINAL_REPORT.md", "01_CODEX_REPORT.md", "02_VALIDATION_REPORT.md", "03_REVIEW_REPORT.md"].includes(requested); const base=meta || legacyRunOutput ? runDir : (workspace ? path.resolve(workspace) : runDir); const target=path.resolve(base,requested); if(!target.startsWith(base+path.sep) && target!==base) throw new Error("Path traversal rejected"); return {runDir,base,target,meta}; }
export async function readRunOutput(runId:string, requested:string, ctx:UiContext={}) { const {base,target,meta}=await resolveRunScopedOutput(runId,requested,ctx); if(meta && meta.readable===false) throw new Error("Output is not readable text"); if(!textOutput(target)) throw new Error("Unsupported output type"); let content=await readFile(target,"utf8"); if(path.extname(target)===".json") content=JSON.stringify(JSON.parse(content),null,2); if(content.length>200_000) content=content.slice(0,200_000)+"\n[truncated]"; return { path: path.relative(base,target), content }; }
export async function resolveRunDownload(runId:string, requested:string, ctx:UiContext={}){ const {target,meta}=await resolveRunScopedOutput(runId,requested,ctx); if(!meta) throw new Error("Output not found in run contract"); return { filePath: target, filename: path.basename(target) }; }
