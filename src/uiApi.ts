...
export async function cloneRepository(input:CloneRepositoryInput, ctx:UiContext={}){ const listed=await listRepositories(ctx); const result=await cloneGithubRepository(input.repositoryId,input.branch,listed.repositories,undefined,ctx.env ?? process.env); return result; }
