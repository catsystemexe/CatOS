...
export async function readRunOutput(runId:string, requested:string, ctx:UiContext={}) { const runDir=path.resolve(runsOf(ctx),runId); const system=await buildUiSystemState(runDir); const workspace=system.raw && typeof system.raw === "object" ? String((system.raw as any).workspacePath ?? (system.raw as any).finalWorkspacePath ?? "") : ""; const base=workspace ? path.resolve(workspace) : runDir; const target=path.resolve(base,requested); if(!target.startsWith(base+path.sep) && target!==base) throw new Error("Path traversal rejected"); const ext=path.extname(target); if(![".md",".txt",".json",".diff",".log",".ts",".tsx",".js",".css",".html",".yaml",".yml"].includes(ext)) throw new Error("Unsupported output type"); let content=await readFile(target,"utf8"); if(ext===".json") content=JSON.stringify(JSON.parse(content),null,2); if(content.length>200_000) content=content.slice(0,200_000)+"\n[truncated]"; return { path: path.relative(base,target), content }; }
