...
export type BranchSummary={name:string;current?:boolean;default?:boolean};
export function getManagedRepositoriesRoot(env:NodeJS.ProcessEnv=process.env):string{return env.CATOS_REPOSITORIES_ROOT || "/home/runner/catos-repositories";}
...
export async function scanManagedRepositories(root:string):Promise<RepositorySummary[]>{ return scanRepositories([path.join(root,"Cloned")],5); }
export async function discoverRepositories(roots:string[],cwd=process.cwd(),maxDepth=3,env:NodeJS.ProcessEnv=process.env):Promise<RepositorySummary[]>{ const current=(await isWorkTree(cwd))?await validateManualRepository(cwd).catch(()=>undefined):undefined; const managed=getManagedRepositoriesRoot(env); const repos=[...await scanRepositories(roots.filter(r=>path.resolve(r)!==path.resolve(managed)),maxDepth),...await scanManagedRepositories(managed)]; const byPath=new Map(repos.map(r=>[r.path,r])); if(current)byPath.set(current.path,current); return [...byPath.values()].sort((a,b)=>{ if(current){ if(a.path===current.path)return -1; if(b.path===current.path)return 1;} return a.name.localeCompare(b.name)||a.path.localeCompare(b.path);}); }
