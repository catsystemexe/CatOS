...
function redact(s: unknown) {
  return String(s ?? "")
    .replace(/(?:OPENAI|CODEX|GITHUB|GH)_[A-Z0-9_]*=\S+/g, "SECRET=<redacted>")
    .replace(/(?<![A-Za-z0-9._-])(?:\/(?:home|workspace|tmp|var|private|mnt|opt|root|Users|run)\/[^\s"'`)]+)/g, "<path>")
...
export async function ensureStepResultArtifacts(runDir: string): Promise<Array<{label:string;path:string;kind:"step-report";readable:true}>> {
  ...
  const expectedFiles = Array.from(new Set([...(Array.isArray(taskBrief?.expectedFiles) ? taskBrief.expectedFiles : []), ...criteria.filter((c:string)=>/^[\w./-]+$/.test(c) && c.includes("/"))]));
