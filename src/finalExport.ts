...
function redact(s: unknown) { return String(s ?? "").replace(/[A-Z]:\\[^\s"]+|\/[A-Za-z0-9._~+@%=-][^\s"]*/g, (m) => m.includes("AUTOCODEX") || !m.includes("/") ? m : "<path>").replace(/(?:OPENAI|CODEX|GITHUB|GH)_[A-Z0-9_]*=\S+/g, "SECRET=<redacted>"); }
...
export async function ensureStepResultArtifacts(runDir: string): Promise<Array<{label:string;path:string;kind:"step-report";readable:true}>> {
  ...
  const diffCheck = validation?.results?.find((r:any)=>/diff.*check/i.test(`${r.name} ${r.command}`));
  ...
  const allNonPass = checks.length > 0 && checks.every((r:any)=>r.status === "SKIPPED" || r.status === "BLOCKED");
  ...
  const reviewMd = ["# REVIEW Report","","## Status",`- status: ${reviewRow?.status ?? "waiting"}`,`- duration: ${reviewRow?.durationMs ?? duration(reviewRow?.startedAt,reviewRow?.finishedAt)}`,"","## Acceptance criteria",...safeList(criteria),"","## Evidence inspected",`- coding result: ${coding ? "available" : "missing"}`,`- validation report: ${validation ? "available" : "missing"}`,`- workspace diff: ${diff.trim()?"available":"empty or missing"}`,`- workspace status: ${wsStatus.trim()?"available":"empty or missing"}`,`- expected output files: ${criteria.join(", ") || "not derived"}`,`- actual changed files: ${changed.join(", ") || "none"}`,"","## Findings",...(findings.length?findings:["- No specific findings were recorded."]),"","## Decision",decision,"","## Reason",review?.summary ?? "Review did not record a reason.",...(decision==="REWORK"?["","## Rework instructions",...(review?.blockingFindings?.length?review.blockingFindings.map((f:any)=>`- ${f.requiredChange}`):["- Address the blocking review findings and rerun validation."])]:[]),""] .join("\n");
...
export async function writeFinalReport(runDir: string): Promise<string> {
  ...
  const md=["# AutoCodex Final Report","","## Run",`- run ID: ${session?.runId ?? input?.runId ?? finalResult?.runId ?? path.basename(runDir)}`,`- repository: ${session?.git?.repositoryPath ? "<repository>" : (input?.repositoryPath ? "<repository>" : "-")}`,`- branch: ${session?.git?.runBranch ?? session?.branch ?? input?.baseBranch ?? "-"}`,`- terminal status: ${status}`,`- duration: ${rows.reduce((a,r)=>a+(r.durationMs??0),0) || "-"}`,"","## Task",input?.goal ?? input?.task ?? session?.goal ?? "-","","## Steps",...rows.map(r=>[`### ${r.name}`,`- status: ${r.status}`,`- duration: ${r.durationMs ?? "-"}`,`- report: ${STEP_REPORTS[r.index-1] ?? "-"}`].join("\n")),"","## Changed files",...safeList(changed,"- none recorded"),"","## Result",finalResult?.terminalMessage ?? `Run ended with ${status}.`,"","## Error",finalResult?.error?clip(JSON.stringify(finalResult.error,null,2), 2000):"- none","","## Final response",finalResult?.finalResponse ?? "-",""] .join("\n");
