...
function taskBriefForStep(step: Step, prompt: string): TaskBrief {
  return {
    objective: step.title,
    acceptanceCriteria: [step.request],
    nonGoals: ["Do not push, merge, or create a remote PR."],
...
function runtimeStatus(error: unknown): {
  ...
  return /timed out|timeout/i.test(message)
    ? {
        status: "timed_out",
        resultStatus: "runtime-timeout",
      }
    : {
        status: "failed",
        resultStatus: "runtime-failure",
      };
}
...
async function runFakeChild(
    ...
    `${JSON.stringify(
      {
        ...
        githubCredentialsRemoved: true,
        sshAgentRemoved: true,
      },
      null,
      2,
    )}\n`,
  ...
  await writeFile(
    path.join(runtimeDir, "codex-runtime.stdout.log"),
    "fake-child stdout\n",
    "utf8",
  );

  await writeFile(
    path.join(runtimeDir, "codex-runtime.stderr.log"),
    "",
    "utf8",
  );

  return {
    threadId: `fake-thread-${path.basename(attemptPath)}`,
    finalResponse: "Fake child completed without modifying files.",
    workspacePath,
    changedFiles: [],
    diff: "",
    status: "",
...
export async function runStepCommand(
  ...
  if (!["open", "awaiting_decision"].includes(step.status)) {
    throw new Error(
      `run-step is not allowed while step ${step.stepId} is ${step.status}.`,
    );
  }

  const taskInput = await readJson<TaskInput>(
  ...
  const generated = await writeContinuePackage(runDir, new Date(), {
    cwd: root,
  });
  ...
  if (!workspacePath && !fake) {
    throw new Error(
      `Session ${session.sessionId} does not define workspacePath; ` +
        "run-step cannot start Codex.",
    );
  }
  ...
  const validationRunner =
    options.validationRunner ?? new ShellValidationRunner();
  ...
  try {
    ...
    if (fake) {
      codingResult = await runFakeChild(
    ...
    const validationReportPath = await writeValidationReport(
      thisAttemptDir,
      validationReport,
    );
    ...
    await copyIfExists(
      ...
      path.join(runDir, "validation-report.json"),
    );

    const reviewReport = fake
      ? {
          schemaVersion: 1 as const,
          verdict: "ACCEPT" as const,
          summary: "Fake child smoke review fixture.",
    ...
    await copyIfExists(
      reviewReportPath,
      path.join(runDir, "review-report.json"),
    );

    const runtimeSource = fake
      ? path.join(thisAttemptDir, "runtime")
      : path.join(path.dirname(codingResult.workspacePath), "runtime");

    const runtimeTarget = path.join(thisAttemptDir, "runtime");

    const runtimeManifest = await copyIfExists(
      path.join(runtimeSource, "runtime.json"),
      path.join(runtimeTarget, "runtime.json"),
    );

    const runtimeStdout = await copyIfExists(
      path.join(runtimeSource, "codex-runtime.stdout.log"),
      path.join(runtimeTarget, "codex-runtime.stdout.log"),
    );

    const runtimeStderr = await copyIfExists(
      path.join(runtimeSource, "codex-runtime.stderr.log"),
      path.join(runtimeTarget, "codex-runtime.stderr.log"),
    );

    const runtimeError = await copyIfExists(
      path.join(runtimeSource, "codex-runtime-error.json"),
      path.join(runtimeTarget, "codex-runtime-error.json"),
    );

    const resultStatus =
      validationReport.status !== "PASS"
        ? "validation-failure"
        : reviewReport.verdict === "REWORK"
          ? "review-rework"
          : reviewReport.verdict === "HUMAN_REQUIRED"
            ? "human-required"
            : "completed";
    ...
    await completeAttempt({
      ...
      workspacePath: codingResult.workspacePath,
      artifacts: artifactRefs(runDir, {
        ...codingArtifacts,
        validationReportPath,
        reviewReportPath,
        runtimeDir: runtimeTarget,
        runtimeManifestPath: runtimeManifest,
        runtimeStdoutPath: runtimeStdout,
        runtimeStderrPath: runtimeStderr,
        runtimeErrorPath: runtimeError,
      }),
    });

    const reviewPackageResult = await writeReviewPackage(runDir).catch(
      (error: unknown) => ({
    ...
    const reviewPackagePath =
      "markdownPath" in reviewPackageResult
        ? reviewPackageResult.markdownPath
        : undefined;

    if ("error" in reviewPackageResult) {
      console.warn(
        `Review Package refresh failed: ${
          reviewPackageResult.error instanceof Error
            ? reviewPackageResult.error.message
            : String(reviewPackageResult.error)
        }`,
      );
    }
    ...
    console.log(
      `Changed files: ${
        codingResult.changedFiles.length
          ? codingResult.changedFiles.join(", ")
          : "none"
      }`,
    );
    ...
    console.log(
      `Review package: ${
        reviewPackagePath ??
        `not refreshed; run npm run catos -- review --run ${runId}`
      }`,
    );
    console.log(
      `Next step: npm run catos -- decide --run ${runId}`,
    );
  } catch (error) {
    ...
    const runtimeErrorPath = path.join(
      runtimeDir,
      "codex-runtime-error.json",
    );

    if (!(await exists(runtimeErrorPath))) {
      await writeFile(
        runtimeErrorPath,
        `${JSON.stringify(
          {
    ...
    await completeAttempt({
      ...
      artifacts: artifactRefs(runDir, {
        runtimeDir,
        runtimeErrorPath,
        runtimeManifestPath: path.join(runtimeDir, "runtime.json"),
        runtimeStdoutPath: path.join(
          runtimeDir,
          "codex-runtime.stdout.log",
        ),
        runtimeStderrPath: path.join(
          runtimeDir,
    ...
    console.log(
      `Runtime error artifact: ${path.relative(
        runDir,
        runtimeErrorPath,
      )}`,
    );
