...
export type Session = { schemaVersion: 1; sessionId: string; runId: string; goal: string; status: SessionStatus; createdAt: string; updatedAt: string; branch: string; workspacePath?: string; steps: string[]; activeStepId?: string };
...
export async function createSession(input: { runDir: string; runId: string; goal: string; branch: string; workspacePath?: string; requestTitle?: string; request?: string; now?: Date }): Promise<{ session: Session; step: Step }> {
  ...
  const session: Session = { schemaVersion: 1, sessionId: input.runId, runId: input.runId, goal: input.goal, status: "active", createdAt, updatedAt: createdAt, branch: input.branch, workspacePath: input.workspacePath, steps: [stepId], activeStepId: stepId };
...
export async function loadStep(runDir: string, stepId?: string): Promise<Step> { const session = await loadSession(runDir); const id = stepId ?? session.activeStepId ?? session.steps[session.steps.length - 1]; const dirs = await import("node:fs/promises").then(fs => fs.readdir(path.join(runDir, "steps"))); const match = dirs.find(d => d.endsWith(`-${id}`)); if (!match) throw new Error(`Step not found: ${id}`); return readJson<Step>(path.join(runDir, "steps", match, "step.json")); }
...
function isClosedStep(status: StepStatus): boolean { return status === "accepted" || status === "rejected" || status === "superseded"; }
async function appendStepStatusChanged(runDir: string, sessionId: string, stepId: string, from: StepStatus, to: StepStatus, timestamp = nowIso()): Promise<void> {
  if (from !== to) await appendTimeline(runDir, { type: "step.status_changed", sessionId, stepId, metadata: { from, to } }, timestamp);
}
export async function createStep(input: { runDir: string; title: string; request: string; supersedeCurrent?: boolean; now?: Date }): Promise<{ session: Session; step: Step }> {
  ...
  if (session.activeStepId) {
    ...
    if (!isClosedStep(current.status)) {
      if (!input.supersedeCurrent) throw new Error(`Cannot create a new step while active step ${current.stepId} is ${current.status}. Decide it first or use --supersede-current.`);
      const from = current.status;
      current.status = "superseded";
      current.completedAt = createdAt;
      await persistStep(input.runDir, current);
      await appendStepStatusChanged(input.runDir, session.sessionId, current.stepId, from, current.status, createdAt);
      await appendTimeline(input.runDir, { type: "step.completed", sessionId: session.sessionId, stepId: current.stepId, metadata: { status: current.status, reason: "supersede-current" } }, createdAt);
    }
  }
  const stepId = newId("step");
  const step: Step = { schemaVersion: 1, stepId, order: session.steps.length + 1, title: input.title, request: input.request, status: "open", createdAt, attempts: [], decisionIds: [] };
  const updatedSession: Session = { ...session, status: "active", steps: [...session.steps, stepId], activeStepId: stepId };
...
export async function startAttempt(input: { runDir: string; step: Step; prompt: string; runtimeMode?: string; now?: Date }): Promise<Attempt> {
  const session = await loadSession(input.runDir);
  if (isClosedStep(input.step.status)) throw new Error(`Cannot start attempt for ${input.step.status} step ${input.step.stepId}.`);
  if (!session.activeStepId) throw new Error("Cannot start attempt: session has no active step.");
  if (session.activeStepId !== input.step.stepId) throw new Error(`Cannot start attempt for inactive step ${input.step.stepId}; active step is ${session.activeStepId}.`);
  if (input.step.status === "running") throw new Error(`Cannot start attempt: step ${input.step.stepId} is already running.`);
  ...
  const from = input.step.status; input.step.status = "running"; input.step.attempts.push(attempt.attemptId); await persistStep(input.runDir, input.step);
  await appendStepStatusChanged(input.runDir, session.sessionId, input.step.stepId, from, input.step.status, attempt.startedAt);
  ...
  await appendTimeline(input.runDir, { type: "attempt.started", sessionId: session.sessionId, stepId: input.step.stepId, attemptId: attempt.attemptId, metadata: { order: attempt.order, runtimeMode: attempt.runtimeMode } }, attempt.startedAt);
  return attempt;
}
export async function completeAttempt(input: { runDir: string; step: Step; attempt: Attempt; status: AttemptStatus; codexThreadId?: string; resultStatus?: string; changedFiles?: string[]; validationSummary?: string; errorSummary?: string; artifacts?: ArtifactRefs; workspacePath?: string; now?: Date }): Promise<Attempt> {
  ...
  const session = await loadSession(input.runDir); const from = input.step.status; input.step.status = "awaiting_decision"; await persistStep(input.runDir, input.step); await appendStepStatusChanged(input.runDir, session.sessionId, input.step.stepId, from, input.step.status, completedAt); await writeJson(path.join(attemptDir(input.runDir, input.step, updated), "attempt.json"), updated);
  await persistSession(input.runDir, { ...session, workspacePath: input.workspacePath ?? session.workspacePath });
...
export async function recordDecision(input: { runDir: string; step?: Step; type: DecisionType; reason?: string; actor?: DecisionActor; attemptId?: string; now?: Date }): Promise<Decision> {
  const session = await loadSession(input.runDir); const step = input.step ?? await loadStep(input.runDir); const createdAt = nowIso(input.now);
  if (isClosedStep(step.status) && input.type !== "commit") throw new Error(`Cannot record decision for ${step.status} step ${step.stepId}.`);
  const decision: Decision = { schemaVersion: 1, decisionId: newId("decision"), createdAt, actor: input.actor ?? "human", type: input.type, reason: input.reason, stepId: step.stepId, attemptId: input.attemptId };
  ...
  step.decisionIds.push(decision.decisionId); step.decisionId = decision.decisionId;
  step.status = input.type === "commit" && isClosedStep(step.status) ? step.status : input.type === "accept" || input.type === "commit" ? "accepted" : input.type === "reject" || input.type === "abort" ? "rejected" : input.type === "revise" ? "superseded" : "open";
  if (["accept","reject","abort","commit","revise"].includes(input.type)) step.completedAt = createdAt; await persistStep(input.runDir, step);
  await appendStepStatusChanged(input.runDir, session.sessionId, step.stepId, from, step.status, createdAt);
  const nextStatus: SessionStatus = input.type === "accept" ? "ready_for_review" : input.type === "commit" ? "committed" : input.type === "abort" ? "aborted" : session.status; const nextActiveStepId = isClosedStep(step.status) && session.activeStepId === step.stepId ? undefined : session.activeStepId; await persistSession(input.runDir, { ...session, status: nextStatus, activeStepId: nextActiveStepId });
  ...
  if (isClosedStep(step.status)) await appendTimeline(input.runDir, { type: "step.completed", sessionId: session.sessionId, stepId: step.stepId, metadata: { status: step.status } }, createdAt);
