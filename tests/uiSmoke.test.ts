...
test("UI page loads repository-first SETUP without project/profile chrome", async()=>{ await withUiServer(async(baseUrl)=>{ const res=await fetch(`${baseUrl}/`); const html=await res.text(); expect(res.status).toBe(200); expect(html).toContain("SETUP"); expect(html).toContain("Git repository"); expect(html).toContain("Branch"); expect(html).toContain("Base branch"); expect(html).not.toContain("PR target"); expect(html).toContain("Task"); expect(html).toContain("RUN"); expect(html).toContain("<h2>SETUP</h2>"); expect(html).toContain("<h2>OUTPUT</h2>"); expect(html).toContain("Local clone"); expect(html).toContain('id="clone"'); expect(html).toContain('id="repositoryPath" readonly'); expect(html).toContain('id="baseBranch" readonly'); expect(html).toContain('<textarea id="task"></textarea>'); expect(html).toContain("STOP"); expect(html).toContain('class="app-layout"'); expect(html).toContain('class="setup-window"'); expect(html).toContain('class="execution-window"'); expect(html).toContain('<script type="module" src="/app.js">'); expect(html).not.toContain("Project status"); expect(html).not.toContain("Profile"); expect(html).not.toContain("Sandbox"); expect(html).not.toContain("F1 Help"); }); });
